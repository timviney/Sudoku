...
def solve(sudoku: Sudoku):
    ...
    for row in range(1, size+1):
        for col in range(1, size+1):
            variables_in_constraint: list[Variable] = [cast(Variable, variables[row, col, val]) for val in range(1, size+1)]
    ...
    for row in range(1, size+1):
        for val in range(1, size+1):
            variables_in_constraint2: list[Variable] = [cast(Variable, variables[row, col, val]) for col in range(1, size+1)]
    ...
    for val in range(1, size+1):
        for col in range(1, size+1):
            variables_in_constraint3: list[Variable] = [cast(Variable, variables[row, col, val]) for row in range(1, size+1)]
            multipliers3: list[float] = [1.0 for m in range(0, size)]
            add_constraint(model, variables_in_constraint3, multipliers3, 1, 1)

    solver: Solver = Solver("SCIP")
    solver.solve(model)
